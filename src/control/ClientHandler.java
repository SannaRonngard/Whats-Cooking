...
public class ClientHandler implements Initializable {
	...
	@FXML
	private CheckBox checkBox_Chicken, checkBox_Lamb, checkBox_Pork, checkBox_Game, checkBox_Rabbit,
	checkBox_Beef, checkBox_Duck, checkBox_Ground_Beef, checkBox_Fish, checkBox_Shrimp, checkBox_Ground_Pork, 
	checkBox_Egg;
	/* Checkboxes - Category Dairy */
	@FXML
	private CheckBox checkBox_Milk, checkBox_Cream, checkBox_Sour_Cream, checkBox_Creme_Fraiche,
	checkBox_Yogurt, checkBox_Hard_Cheese, checkBox_Soft_Cheese, checkBox_Cottage_Cheese,
	checkBox_Blue_Cheese, checkBox_Parmesan, checkBox_Butter;
	/* Checkboxes - Category Grains */
	@FXML
	private CheckBox checkBox_Wheat, checkBox_Oats, checkBox_Barley, checkBox_CornMeal,
	checkBox_Rye, checkBox_Rice, checkBox_Bulgur, checkBox_Couscous, checkBox_Cracked_Wheat,
	checkBox_Bread, checkBox_Pasta;
	...
	checkBox_Lemon, checkBox_Lime, checkBox_Banana, checkBox_Strawberry, checkBox_Blueberry, checkBox_Blackberry,
	checkBox_Cherry, checkBox_Pineapple, checkBox_Apricot, checkBox_Grapes, checkBox_Mango, checkBox_Papaya, checkBox_Kiwi;
	/* Checkboxes - Category Vegetables */
	@FXML
	private CheckBox checkBox_Tomato, checkBox_Cucumber, checkBox_Pepper, 
	checkBox_Zucchini, checkBox_Aubergine, checkBox_Pumpkin, checkBox_Avocado, 
	checkBox_Onion, checkBox_Garlic, checkBox_Spring_Onion, checkBox_Leek, checkBox_Celery,
	checkBox_Aspargus, checkBox_Artichoke, checkBox_Potato, checkBox_Sweet_Potato, checkBox_Radish,
	checkBox_Parsnip, checkBox_Carrot, checkBox_Cabbage, checkBox_Cauliflower, checkBox_Lettuce, 
	checkBox_Spinach, checkBox_Sweetcorn, checkBox_Green_Peas, checkBox_Chick_Peas, checkBox_White_Beans, 
	checkBox_Black_Beans, checkBox_Coriander, checkBox_Tarragon, checkBox_Parsley, checkBox_Dill, checkBox_Chili,
	checkBox_Lemongrass;
	@FXML
...
//	}
	/**
	 * Methods that saves checked item to a list or removes it from the list whenever
	 * box is unchecked.
	 */
	@FXML
	private void handleCheckBoxActionMeat(ActionEvent event) throws IOException {
				...
				if (checkBox_Chicken.isSelected()==true) {
					if (!listMeat.contains("Chicken")) {listMeat.add("Chicken");}
			 	}
				if (checkBox_Chicken.isSelected()==false) {
					if (listMeat.contains("Chicken")) {listMeat.remove("Chicken");}
		 		}
		 
			 	if (checkBox_Lamb.isSelected()==true) {
			 		if (!listMeat.contains("Lamb")) {listMeat.add("Lamb");}
			 		}
			 		if (checkBox_Lamb.isSelected()==false) {
			 			if (listMeat.contains("Lamb")) {listMeat.remove("Lamb");}
			 		}
	
			 		
			returnBtn.setOnAction(e -> {System.out.println(listMeat);});
	...
	private void handleCheckBoxActionVeg(ActionEvent event) throws IOException {
			...
			if (checkBox_Avocado.isSelected()==true) {
				if (!listVeggies.contains("avocado")) {listVeggies.add("avocado");}
			}
			if (checkBox_Avocado.isSelected()==false) {
				if (listVeggies.contains("avocado")) {listVeggies.remove("avocado");}
			}
